...
constructor(private val application: Application, private val marvelRepo : MarvelRepo) {
    ...
    fun validateCharacters(url : String,type : Int) {
        if (compositeDisposable.isDisposed)
            compositeDisposable = CompositeDisposable()
        compositeDisposable.add(marvelRepo.getDetailItemListing(url )
            ...
            .subscribe(
                { data ->
                    data?.let {
                        ...
                        validateMarvelResponseData.postValue( Event (Pair(it,type)))
                        Timber.e(""+it)
                    } ?: run {
